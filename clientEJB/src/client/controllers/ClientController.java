...
public class ClientController extends IObserver implements Controller {
    ...
    public void xhsUpdateStructureTable(Long id) {
        Thread t = new Thread() {
            public void run() {
                //this will set and reload the site structure tables
                siteController = SiteController.GetInstance();
                List<Structure> s = siteController.getStructures(id);

                model.setStructures(s);
